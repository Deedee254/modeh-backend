...
class ChatController extends Controller
{
    ...
    public function send(Request $request)
    {
        $request->validate([
            ...
            'attachments.*' => 'file|max:10240|mimes:jpg,jpeg,png,gif,webp,pdf,doc,docx,txt,mp3,mp4,zip',
        ]);
    ...
    private function updateMetrics(int $count = 1): void
    {
        try {
            ...
            $metricBucket = \App\Models\ChatMetricBucket::firstOrCreate(
                ['metric_key' => 'messages_per_minute', 'bucket' => $bucket],
                ['value' => 0, 'last_updated_at' => now()]
            );
            $metricBucket->increment('value', $count, ['last_updated_at' => now()]);
