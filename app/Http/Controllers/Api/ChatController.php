...
class ChatController extends Controller
{
    ...
    public function messages(Request $request)
    {
        ...
        if ($request->has('group_id')) {
            ...
            $isMember = \App\Models\Group::where('id', $gid)->whereHas('members', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            })->exists();
            if (! $isMember) {
    ...
    private function createMessage(int $fromId, Request $request, ?array $attachmentsMeta): Message
    {
        if ($request->filled('group_id')) {
            $gid = intval($request->input('group_id'));
            $isMember = \App\Models\Group::where('id', $gid)->whereHas('members', function ($q) use ($fromId) {
                $q->where('users.id', $fromId);
            })->exists();
            if (! $isMember) {
                abort(403, 'Forbidden');
            }
