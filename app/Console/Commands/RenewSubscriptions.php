...
class RenewSubscriptions extends Command
{
    ...
    public function handle()
    {
        ...
        $subs = Subscription::with('package', 'user')
        ...
        foreach ($subs as $sub) {
            ...
            if (! $phone) {
                $this->error('No phone for subscription '.$sub->id);
            ...
            if ($res['ok']) {
                ...
                $gateway_meta = is_array($sub->gateway_meta) ? $sub->gateway_meta : [];
                $gateway_meta['last_renew_tx'] = $res['tx'];
                $gateway_meta['renew_initiated_at'] = now();

                // Use DB::update instead of model save
                \DB::table('subscriptions')
                    ->where('id', $sub->id)
                    ->update([
                        'gateway_meta' => json_encode($gateway_meta),
                        'updated_at' => now(),
                    ]);
